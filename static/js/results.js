...
async function generateRoadmap() {
    try {
        ...
        if (!assessmentId) {
            alert('Assessment ID not found. Please complete an assessment first.');
        ...
        if (response.ok) {
            ...
            // Store roadmap in localStorage for roadmap page
            localStorage.setItem('latestRoadmap', JSON.stringify(data.roadmap));

            // Show success message
            alert('✅ Roadmap generated successfully! Resources auto-generated by AI.');

            // Redirect to roadmap page
            window.location.href = '/roadmap/';
        } else {
            const error = await response.json();
            alert(`Failed to generate roadmap: ${error.error || 'Unknown error'}`);
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = '📚 Generate Learning Roadmap';
        }
    }
}
